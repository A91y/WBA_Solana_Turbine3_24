...
declare_id!("DWZLW6oprmJsbjeE1H5dzNmmk7y2rQHY2UHaKpyVhixc");
...
pub mod vote_program {
    ...
    pub fn upvote(ctx: Context<Vote>, _url: String) -> Result<()> {
        ctx.accounts.upvote(ctx.accounts.signer.key())?;

        Ok(())
    }

    pub fn downvote(ctx: Context<Vote>, _url: String) -> Result<()> {
        ctx.accounts.downvote(ctx.accounts.signer.key())?;
...
impl<'info> Vote<'info> {
    pub fn upvote(&mut self, signer_key: Pubkey) -> Result<()> {
        self.vote_account.score += 1;
        self.vote_account.last_vote = Some(signer_key);
        Ok(())
    }

    pub fn downvote(&mut self, signer_key: Pubkey) -> Result<()> {
        self.vote_account.score -= 1;
        self.vote_account.last_vote = Some(signer_key);
        Ok(())
    }
